...
static void li2a(long num, char * bf,char sign) {
	if (num < 0) {
		num = -num;
		*bf++ = '-';
	} else if(sign) {
...
static void i2a(int num, char * bf,char sign) {
	if (num < 0) {
		num = -num;
		*bf++ = '-';
	} else if(sign) {
...
static void putchw(void* putp, putcf putf, int minmax, char z, char* bf) {
	char fc = z ? '0' : ' ';
	char ch;
	char* p = bf;
	int n = minmax;
...
#ifdef  PRINTF_LONG_SUPPORT
				if (lng)
				li2a(va_arg(va, unsigned long int),bf,sign);
				else
#endif
				i2a(va_arg(va, int),bf,sign);
				putchw(putp, putf, w, lz, bf);
				break;
			}
			case 'x':
...
void tiny_dprintf(const char *fmt, ...) {
	va_list va;
	va_start(va, fmt);
	tfp_format(NULL, serial_putc, fmt, va);
	va_end(va);
}
